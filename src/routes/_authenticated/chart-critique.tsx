...
function ChartCritique() {
  ...
  const { run, checking } = useAiAction("chart-critique");

  async function critique() {
    const ok = await run();
    if (ok) setScored(true);
  }
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <Panel className="lg:col-span-2" title="Pre-trade setup">
          <ChartUpload
            value={chart}
            height="h-48"
            hint="Upload the chart you're about to trade"
            onChange={(c) => {
              setChart(c);
              setScored(false);
            }}
          />
          <Button className="mt-3 w-full" disabled={!chart || checking} onClick={() => void critique()}>
            {checking ? "Checking…" : "Critique this setup"}
          </Button>
        </Panel>
        ...
        <Panel title="Execution score">
          {scored ? (
            ...
            <EmptyHint>Upload your pre-trade chart to get a setup score.</EmptyHint>
          )}
        </Panel>
      </div>
